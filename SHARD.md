...
| `nav-hop` (token only) | Administrator | Is this hop walkable, and where is the nearest road. Body is `verify x,y[,z] x,y[,z] ...` (pairs, each a hop) and/or `snap x,y`, plus an optional **`bot`** or **`creature`** to choose the probe class. **It defaults to `bot`**, the fleet's own class, because a `BaseCreature` probe is granted `IgnoreMovableImpassables` and walks through crates a bot must go round - 297 expansions and success against 301 and failure on the same tiles. `creature` is the right answer for the daily-life walkers. Answers to `Data/Live/nav-hop.json`; `Nav.HopClass` fails if the two classes ever stop parting |
...
| `Custom.NavPathfinder` | `Off` | Which pathfinder instrument is installed behind `MovementPath.OverrideAlgorithm`. **NOT A LIVE-SHARD SETTING** - every number this shard has recorded assumes `Off`, and `Nav.Pathfinder` Fails if the override is not null while it says `Off`. `Meter` is a pass-through counter over stock for every caller; `Mirror` and `DeadEndFix` run a `Custom/` copy of `FastAStarAlgorithm` **for bot subjects only** and delegate everything else verbatim, which is what makes the creature walk audit provably unchanged. The `nav-pathfinder` token can override the mode and the budget **for one boot** - the file stays the authority for what the shard boots with, and the check reports Warn for as long as an override is in force. **The `[Path` caveat**: `MovementPath.Path_OnTarget` clears the override at `MovementPath.cs:126`, so a GM running `[Path` would silently revert the shard to stock; `Nav.Pathfinder` reinstalls and counts instead, bounding the exposure at one sixty-second health interval and naming it in the health line. |
| `Custom.NavPathfinderBudget` | 300 | Node budget for that copy, ignored when `NavPathfinder` is `Off`. It does **not** change the real one - `FastAStarAlgorithm.cs:26` is a `private const` on a class whose every scratch array is `private static`, which is why the copy exists. ModernUO's descendant of the same file runs 1000. Measured here: 600 rescues three hops on the whole graph and 1000 rescues none beyond that. |
| `Custom.LoopCostSampler` | False | Sample the whole game loop's cycle period and report p50/p95/p99/max on `Core.Process`. Hooks `Core.Slice`, which nothing else in the tree subscribes to, so it needs no upstream edit. The `loop-cost` token takes `on`, `off` and `reset` so a window can be opened on a warm shard. See the Navigation README, *`LoopCost` - the whole game loop*. |
