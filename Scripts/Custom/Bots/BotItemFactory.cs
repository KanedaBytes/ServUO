...
//
// Two traps, both learned the hard way upstream, both still true here.
//
// TRAP 1 - the single-int constructor. Item types are inconsistent about what
// one int means. On stackables (Gold, Bandage, Kindling, Log, Ingot) it is an
// amount; everywhere else it is a hue. Passing 0 blindly builds Amount = 0
// stacks, which makes the engine log "Item.Amount <= 0" once per item per
// spawn wave. Inspect the parameter's NAME and pass something safe for what it
// actually is.
//
...
//
// Every item type the outfit table names lives in Server.Items in this tree
// too, so the fully-qualified names carry over from upstream unchanged.
