...
// -----------------------------------------------------------------------------
//
// EquipmentTable.cs — what a bot is wearing and carrying.
//
// Three stages, in order: the class look (armour archetype and weapon, weighted
// by class), universal accessories, and a footwear safety net so nothing ever
// walks around barefoot by accident. Then the backpack: gold scaled to tier,
// class-appropriate consumables, and enough odds and ends that a bot is worth
// snooping and drops a real corpse.
//
// STILL T2A-FLAVOURED. This shard is EJ, and the tables here are the ones
// upstream curated for 1998-99 - no wakizashi, pre-AOS magic tiers, era-sized
// reagent stashes. Every item type in it exists in this tree, so nothing is
// broken by that; it is a flavour mismatch, not a compatibility one. Rebalancing
// it for EJ is a deliberate later data pass, and the right shape for that pass is
// to lift these tables out into Data/Custom/ rather than to edit them in place.
//
// Two of the five severed seams live in this file - see AddMarkedRune and
// SeedCrafterStarterProps, and README.md for the full list.
...
namespace Server.Custom
{
    public static class EquipmentTable
    {
        ...
        /// <summary>Dress and load a bot from its own class and tier.</summary>
        public static void RollOutfit(PlayerBot bot)
        {
            RollOutfit(bot, bot.Class, bot.SkillTier);
        }
        ...
        private static void RollBackpackLoot(PlayerBot bot, BotClass cls, BotSkillTier tier)
        {
            ...
            if (bot.Backpack != null &&
                bot.Skills[SkillName.Magery].Base >= BotSkillTemplates.TravelMageryMin(BotSystem.Caps))
            {
        ...
        // A blank recall rune.
        //
        // SEAM 2 of 5 (see README.md): upstream marked this to a random entry from its
        // DestinationCatalog, which is the travel layer's. Ours is Nav, and nothing routes yet -
        // a rune marked to a destination no bot can walk to would be a lie in the pack. It is
        // wired to Nav.Destinations(...) when the travel session lands; until then a bot carries
        // an unmarked rune, which is what a player who has not been anywhere yet carries.
        private static void AddMarkedRune(PlayerBot bot)
        {
            try
            {
                bot.AddToBackpack(new RecallRune());
            }
            catch
            {
            }
        ...
        private static void PackSpareTankWeapon(PlayerBot bot, BotSkillTier tier)
        {
            ...
            if (best < BotSkillTemplates.TankWeaponSkillMin(BotSystem.Caps))
            {
        ...
        private static void MaybeEnchantWeapon(BaseWeapon w, BotSkillTier tier)
        {
            ...
            {
                w.Quality = ItemQuality.Exceptional;
            }
        }
        ...
        private static void MaybeEnchantArmor(BaseArmor a, BotSkillTier tier)
        {
            ...
            if (Utility.RandomDouble() < 0.35 + 0.09 * rank)
            {
                a.Quality = ItemQuality.Exceptional;
            }
        }
